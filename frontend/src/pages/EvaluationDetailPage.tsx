...
export function EvaluationDetailPage() {
  ...
  const [category, setCategory] =
    useState<BadCaseCategory | null>(null)

  const { data: items, isLoading } = userEvaluationItems(runId, {
    page,
    pageSize,
    badCaseOnly,
    category,
  ...
  const [activeItem, setActiveItem] =
    useState<EvaluationItemRead | null>(null)
  ...
  return (
    <div>
      <Space style={{ marginBottom: 16 }}>
        ...
        {run && (
          <Text type="secondary">
            评测集 {run.dataset_name}（{run.dataset_size} 条）
            · 状态 {run.status}
            {run.status === 'running' &&
      ...
      <Row gutter={[12, 12]} style={{ marginBottom: 16 }}>
        <Col xs={12} md={6}>
          <MetricsCard
            label="Faithfulness"
            value={run?.faithfulness}
            hint="答案被引用片段支撑的程度（RAGAS）"
          />
        </Col>
...
function ItemDetailDrawer({
  ...
  const [editedCategory, setEditedCategory] =
    useState<BadCaseCategory | null>(
      () => item?.bad_case_category ?? null,
    )

  const [editedIsBadCase, setEditedIsBadCase] =
    useState(() => item?.is_bad_case ?? false)

  const citations = (
    item?.citations ?? []
  ) as unknown as CitationRead[]

  const queryRoute =
    item?.query_route as unknown as QueryRouteRead | null
  ...
  const verifyResult =
    item?.verify_result as unknown as VerifyResultRead | null
      ...
      {item && (
        <div>
          ...
          >
            ...
            {queryRoute && (
              <QueryRoutePanel queryRoute={queryRoute} />
            )}
            ...
            <Space size={12} wrap>
              ...
              <Text type="secondary">
                ...
                item.first_token_latency_ms !== undefined
