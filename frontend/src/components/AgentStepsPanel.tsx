 ...
 *
 * 只有 1 步且 action=initial 时不渲染一轮检索没有“代理”价值，
...
export function AgentStepsPanel({
  ...
  console.log("steps.length",rounds)
          ...
          label: (
            <span>
              ...
              <Text type="secondary">
                最终动作：
                {finalStep
                  ? ACTION_META[finalStep.action].label
                  : '-'}
              </Text>
            </span>
          ),
...
function StepRow({ step }: { step: AgentStep }) {
  ...
  return (
    <div>
      ...
      {step.retrieved_count !== null ||
      step.top_score !== null ? (
        <Text type="secondary" style={{ fontSize: 12 }}>
          检索 {step.retrieved_count ?? '-'} 条
          {step.top_score !== null
            ? ` · Top score ${step.top_score}`
            : ''}
        </Text>
      ) : null}
    </div>
