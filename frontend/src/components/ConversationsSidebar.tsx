...
interface ConversationSidebarProps {
  ...
  /** 当前会话被删除时回调，由 ChatPage 置当前 id 并清空 pending */
  onDeleted: (deletedId: string) => void

  /** 新建对话，ChatPage 内部已有 createMutation，这里只触发，避免重复 mutation */
  onCreateNew: () => void

  /** 是否正在加载，与 ChatPage 的 createMutation.isPending 联动 */
  isCreating: boolean
}
...
export function ConversationSidebar({
  ...
  const conversationsQuery = useQuery({
    ...
    queryFn: async () => {
      // 一页一百条，侧栏不做无限滚动，超过 100 条的场景留到后续章节
