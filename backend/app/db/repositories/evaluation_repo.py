...
class EvaluationItemRepository():
    ...
    async def list_page(
        ...
        if bad_case_only:
            filter.append(EvaluationItem.is_bad_case.is_(True))
        if category:
            filter.append(EvaluationItem.bad_case_category==category)

        stmt =(
            select(EvaluationItem).where(*filter)
            .order_by(EvaluationItem.created_at.asc(),EvaluationItem.id.asc())
            .limit(page_size)
            .offset(offset)
        )
        ...
        total_stmt = select(func.count(EvaluationItem.id)).where(*filter)
        total = int(
