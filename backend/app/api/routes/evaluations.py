...
@router.get(
    ...
    summary="列出可用评测集（jsonl 文件名和条数）"
)
async def list_evaluation_datasets(session:DbSession)->DatasetListResponse:
    service = EvaluationService(session)
    items = service.list_datasets()
    return DatasetListResponse(items=[DatasetInfo(name=name,size=size) for name,size in items])
...
@router.post(
    ...
    summary="创建评测RUN并通过BackgroundTasks异步执行",
)
async def create_evaluation_run(
    ...
    backgound_tasks.add_task(execute_evaluation_run,run.id)
...
async def list_evaluation_runs(
    ...
    for r in items:
        item = EvaluationRunListItem.model_validate(r)
        logger.warning(item)
...
@router.get(
    ...
    summary="分页列出RUN下的CASE，支持仅看bad case与按归因筛选",
)
...
async def get_evaluation_item(item_id:UUID,session:DbSession)->EvaluationItemRead:
    ...
    result = EvaluationItemRead.model_validate(item)
    logger.info(
...
@router.patch(
    ...
    summary="人工覆盖BAD CASE归因/备注",
)
async def update_evaluation_item(item_id:UUID,payload:EvaluationItemUpdate,
                                 session:DbSession)->EvaluationItemRead:
    service =EvaluationService(session)
    item = await service.update_item_bad_case(
        item_id,bad_case_category=payload.bad_case_category,
