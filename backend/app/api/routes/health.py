...
async def health_cos()->HealthStatus:
    if not settings.cos_configured:
        return HealthStatus(status="not_configured",detail="COS 凭据未在.env中配置")
    try:
    ...
    return HealthStatus(status="ok") if ok else HealthStatus(status="error",detail="head_bucket失败")